...
for (const entry of readdirSync(programsDir, { withFileTypes: true })) {
  ...
  for (let weekIndex = 0; weekIndex < parsed.weeks.length; weekIndex += 1) {
    ...
    if (!Array.isArray(week.days) || week.days.length === 0) {
      errors.push(`${entry.name}: weeks[${weekIndex}].days must be a non-empty array`);
      continue;
    }
    ...
    for (let dayIndex = 0; dayIndex < week.days.length; dayIndex += 1) {
      ...
      if (day === null || typeof day !== 'object' || Array.isArray(day)) {
        errors.push(`${entry.name}: weeks[${weekIndex}].days[${dayIndex}] must be an object`);
        continue;
      }

      if (!Array.isArray(day.blocks) || day.blocks.length === 0) {
        errors.push(`${entry.name}: weeks[${weekIndex}].days[${dayIndex}].blocks must be a non-empty array`);
        continue;
      }
      ...
      for (let blockIndex = 0; blockIndex < day.blocks.length; blockIndex += 1) {
        ...
        if (block === null || typeof block !== 'object' || Array.isArray(block)) {
          errors.push(`${entry.name}: weeks[${weekIndex}].days[${dayIndex}].blocks[${blockIndex}] must be an object`);
          continue;
        }
...
console.log(`Validated ${checkedFiles} program file${checkedFiles === 1 ? '' : 's'}.`);
