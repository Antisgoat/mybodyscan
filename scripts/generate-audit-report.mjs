...
// Runs the audit graph and produces a Markdown report under reports/.
// Outputs:
//   - reports/import-graph.json (raw graph)
//   - reports/REPORT_MyBodyScan_Audit_<timestamp>.md (human-readable summary)
...
if (!graphRun.ok) {
  const errPath = path.join(REPORTS_DIR, `audit-graph-error-${ts}.log`);
  fs.writeFileSync(errPath, graphRun.out);
  console.error("audit-graph failed; see", errPath);
  process.exit(2);
}

const graphJsonPath = path.join(REPORTS_DIR, "import-graph.json");
fs.writeFileSync(graphJsonPath, graphRun.out);
const graph = JSON.parse(graphRun.out);

// firebase.json checks (small curated list)
...
> Generated by scripts/generate-audit-report.mjs. No app code was changed.
...
const reportPath = path.join(REPORTS_DIR, `REPORT_MyBodyScan_Audit_${ts}.md`);
