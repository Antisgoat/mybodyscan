...
function normalize(p) { return path.resolve(p).split(path.sep).join(path.posix.sep); }
...
const baseUrl = tsconfig?.compilerOptions?.baseUrl ? path.resolve(root, tsconfig.compilerOptions.baseUrl) : root;
...
// ----- file set / import graph -----
const filesAbs = fs.existsSync(srcDir) ? walk(srcDir) : [];
const files = filesAbs.map(normalize);
const fileSet = new Set(files);

// import / export / dynamic-import / import.meta.glob
...
const importers = new Map(); // resolvedFile -> Set(importerFile)
const edges = [];
...
function resolveImport(spec, fromFile) {
  ...
  return null; // bare module import → ignore
}
...
// ---- glob support (mini translator for * ** ?) ----
...
// ----- case conflicts (same directory + same basename ignoring case) -----
const caseKey = (f) => {
...
// ----- stray fetch calls (always flag; hint if file also imports unified client) -----
...
for (const abs of filesAbs) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (apiRe.test(line) || fnRe.test(line)) {
      strayFetchCalls.push({ file: rel(f), line: idx + 1, snippet: line.trim().slice(0, 220), alsoUsesUnified: alsoUnified });
    }
  });
}
