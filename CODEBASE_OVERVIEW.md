...
- `deleteMyAccount` / `exportMyData` (`functions/src/account.ts`) – Callable utilities to recursively purge a user’s Firestore tree, clean user Storage folders (`scans/{uid}` and legacy `user_uploads/{uid}`), and return short-lived export URLs for scans.【F:functions/src/account.ts†L10-L184】
