  ...
  return (
    <div className="mx-auto max-w-2xl p-4 space-y-4">
      <header className="sticky top-0 z-40 -mx-4 mb-1 bg-white/80 backdrop-blur border-b px-4 py-2 flex items-center gap-3">
        ...
        <button onClick={runChecks} disabled={busy} className="rounded border px-2 py-1 text-xs">
          {busy ? "Running…" : "Re-run"}
        </button>
      </header>
          ...
          <tbody>
            {envRows.map(([k, v]) => (
              <tr key={k} className="border-b last:border-0">
                <td className="py-1 pr-2 text-muted-foreground whitespace-nowrap">{k}</td>
      ...
      <section className="space-y-2">
        ...
        {!error && !health && <p className="text-xs text-muted-foreground">Waiting for health response…</p>}
        {health && (
          <pre className="rounded border bg-black/5 p-2 text-[11px] overflow-auto">{JSON.stringify(health, null, 2)}</pre>
        )}
        <p className="text-[11px] text-muted-foreground">
          This request includes your Firebase ID token and App Check token (if available). We expect JSON and no “app_check_required” errors during UAT (soft mode). :contentReference[oaicite:4]{index=4}
        </p>
      </section>
      ...
      <section className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          For barcode scanning and camera previews, we require a secure context on web. In the native wrapper, file inputs will open Camera/Library. :contentReference[oaicite:5]{index=5}
        </p>
      </section>
    </div>
  );
