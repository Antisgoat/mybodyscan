...
const goalOptions: Array<{
...
}> = [
  {
    value: "hypertrophy",
    label: "Build muscle",
    description: "Hypertrophy-focused growth",
  },
  {
    value: "strength",
    label: "Get stronger",
    description: "Prioritize heavier compounds",
  },
  {
    value: "cut",
    label: "Lean out",
  ...
  },
  {
    value: "general",
    label: "Stay balanced",
    description: "General fitness and training",
  },
];
...
const levelOptions: Array<{
...
}> = [
  {
    value: "beginner",
    label: "New lifter",
    description: "Under 1 year of training",
  },
  {
    value: "intermediate",
    label: "Intermediate",
    description: "1-3 years under the bar",
  },
  {
    value: "advanced",
    label: "Advanced",
    description: "3+ years with structured training",
  },
];
...
export default function ProgramsQuiz() {
  ...
  const computeReasons = (entry: CatalogEntry, score: number): string[] => {
    ...
    if (meta.level === level) {
      reasons.push(
        `${levelOptions.find((item) => item.value === level)?.label} friendly`
      );
    }
  ...
  return (
    <div className="min-h-screen bg-background pb-16 md:pb-0">
      <Seo
        title="Program quiz – MyBodyScan"
        description="Find the right training plan in under a minute."
      />
        ...
        <Card className="border bg-card/60">
          <CardHeader className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Answer five questions and we’ll recommend the top three blocks for
              your goals.
            </p>
          </CardHeader>
          <CardContent>
            <form className="space-y-6" onSubmit={handleSubmit}>
              <section className="space-y-3">
                ...
                <RadioGroup
                  value={goal}
                  onValueChange={(value) => setGoal(value as ProgramGoal)}
              ...
              <section className="space-y-3">
                ...
                <RadioGroup
                  value={level}
                  onValueChange={(value) => setLevel(value as ProgramLevel)}
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-3 rounded-md border bg-muted/40 px-4 py-5">
                  <Slider
                    ...
                    step={1}
                    value={[daysPerWeek]}
                    onValueChange={(value) => setDaysPerWeek(value[0] ?? 4)}
                  />
                  <p className="text-xs text-muted-foreground">
                    {daysPerWeek} training days (±1 wiggle room)
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                {ownedEquipment.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Select anything you can access, or leave blank for
                    bodyweight only.
                  </p>
                )}
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-3 rounded-md border bg-muted/40 px-4 py-5">
                  <Slider
                    ...
                    step={5}
                    value={[sessionTime]}
                    onValueChange={(value) => setSessionTime(value[0] ?? 45)}
                  />
                  <p className="text-xs text-muted-foreground">
                    Up to {sessionTime} minutes each workout
                  </p>
                </div>
              </section>
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        navigate(`/programs/${item.entry.meta.id}`)
                      }
                    >
                      View details
                    </Button>
