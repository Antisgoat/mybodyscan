...
/**
 * Client-side STAFF/TEST Pro allowlist (UX only; server is authoritative).
 *
 * Optional env overrides:
 * - VITE_STAFF_PRO_UIDS="uid1,uid2"
 * - VITE_STAFF_PRO_EMAILS="a@b.com,c@d.com"
 */
export const STAFF_PRO_UID_ALLOWLIST = new Set<string>([
  ...DEFAULT_STAFF_PRO_UIDS,
  ...parseCsvEnv("VITE_STAFF_PRO_UIDS"),
]);

export const STAFF_PRO_EMAIL_ALLOWLIST = new Set<string>(
  [...DEFAULT_STAFF_PRO_EMAILS, ...parseCsvEnv("VITE_STAFF_PRO_EMAILS")].map((e) =>
    e.toLowerCase()
  )
);
...
export function isStaffProUser(user?: StaffProUserLike | null): boolean {
  if (!user) return false;
  return isStaffProUid(user.uid) || isStaffProEmail(user.email);
}
