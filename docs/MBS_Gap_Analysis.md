...
- **Tasks**:
  - **P0**: Monitor auth bootstrap failures and surface a toast when claims refresh errors repeat.
...
- **Tasks**:
  - **P0**: Add explicit loading/error surface on `/scan/capture` and `/scan/result` when functions or App Check are unavailable.
...
- **Tasks**:
  - **P0**: Confirm webhook deployment and event types keep `subscription.status` in sync; add UI copy for `past_due`/`canceled` states.
...
- **Tasks**:
  - **P0**: Surface inline error banners for chat send failures and clearly gate when `COACH_RPM` env is missing.
...
- **Tasks**:
  - **P0**: Detect missing nutrition env keys and show an inline warning/disabled state.
...
- **Tasks**:
  - **P0**: Add empty-state copy when no plan and generation fails, including retry CTA once env is present.
...
- **Tasks**:
  - **P0**: Add explicit “health data unavailable” copy on Today when no connector is configured.
...
- **Tasks**:
  - **P0**: Audit scan metrics and onboarding inputs to ensure they read from `useUnits` and convert labels/values.
...
- **Tasks**:
  - **P0**: Add a lightweight “Feature availability” panel in settings summarizing detected env/secrets per feature.
