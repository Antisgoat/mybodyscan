...
const VISUAL_PROPORTION_RANGES = {
  ...
  hipDepthToHeight: [0.09, 0.25],
  armWidthToHeight: [0.025, 0.09],
  armLengthToHeight: [0.24, 0.42],
  thighWidthToHeight: [0.045, 0.14],
  calfWidthToHeight: [0.025, 0.085],
  legLengthToHeight: [0.4, 0.58],
  torsoLengthToHeight: [0.25, 0.4],
  neckWidthToHeight: [0.055, 0.12],
  headWidthToHeight: [0.09, 0.17],
  confidence: [0, 1],
} as const;
...
export async function callOpenAI(
  ...
  const userText = [
    ...
    PHYSIQUE_SCORE_MODEL_INSTRUCTION,
    "For visualProportions, estimate visible outer-body ratios relative to standing height using all four views: shoulder, chest, waist, hip, side torso and hip depth, arm width and length, thigh and calf width, leg and torso length, neck width, and head width. Use decimals, not percentages.",
