...
export const systemHealth = onRequest(
  ...
  async (req, res) => {
    ...
    const usdaKeyPresent =
      envPresent(process.env.USDA_API_KEY) ||
    ...
    res.status(200).json({
      ...
      engineConfigured: scanEngine.configured,
      engineMissingConfig: scanEngine.missing,
      engineProvider: scanEngine.provider,
      engineModel: scanEngine.model,
      engineBaseUrl: scanEngine.baseUrl,
      storageBucket: scanEngine.bucket,
      storageBucketSource: scanEngine.bucketSource,
      projectId: scanEngine.projectId,
      coachConfigured,
      workoutsConfigured,
