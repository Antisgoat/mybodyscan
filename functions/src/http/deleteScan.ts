...
export const deleteScan = onRequest({ cors: true, timeoutSeconds: 540 }, async (req, res) => {
  if (req.method === "OPTIONS") return res.status(204).end();
  if (req.method !== "POST") return jsonError(res, 405, "method_not_allowed", "Use POST");

  const authz = String(req.headers.authorization || "");
  const idToken = authz.startsWith("Bearer ") ? authz.slice(7) : "";
  if (!idToken) return jsonError(res, 401, "unauthorized", "Missing ID token");
...
function jsonOk(res:any, data?:any) { withCors(res); res.status(200).json({ ok:true, ...(data?{data}: {}) }); }
function jsonError(res:any, http:number, code:string, message:string) { withCors(res); res.status(http).json({ ok:false, error:{ code, message } }); }
