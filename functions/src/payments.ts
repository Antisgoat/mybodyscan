...
export async function createCheckout(req: Request, res: Response) {
  ...
  const stripe = new Stripe(getStripeSecret() as string, { apiVersion: "2024-06-20" });

  // Minimal example — keep your existing payload building if present:
...
export async function stripeWebhook(req: Request, res: Response) {
  ...
  try {
    ...
    event = Stripe.webhooks.constructEvent((req as any).rawBody, sig, signingSecret);
  } catch (err: any) {
    return res.status(400).json({ error: "invalid_signature", message: err?.message });
  }
  // No-ops are OK for optional payments.
