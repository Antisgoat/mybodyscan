...
export const legacyCreateCheckout = appJson(async (req, res) => {
  ...
  const { kind, credits = 1 } = (req.body || {}) as {
  ...
  if (kind === "scan") {
    if (!priceScan)
      return res
        .status(501)
        .send(JSON.stringify({ error: "price_scan_unset" }));
    mode = "payment";
    line_items = [
      { price: priceScan, quantity: Math.max(1, Number(credits) || 1) },
    ];
    ...
    if (!priceMonthly)
      return res
        .status(501)
        .send(JSON.stringify({ error: "price_monthly_unset" }));
    mode = "subscription";
    ...
    if (!priceAnnual)
      return res
        .status(501)
        .send(JSON.stringify({ error: "price_annual_unset" }));
    mode = "subscription";
