...
function resolveDays(profile) {
    if (!profile)
        return 4;
    if (profile.style === "all_in")
        return 5;
    if (profile.activity_level === "sedentary")
        return 3;
    return 4;
}
function pickSplit(dayCount) {
    if (dayCount <= 3)
...
function buildSessions(dayCount) {
    const base = [
        {
            day: "Day 1",
            blocks: [
...
function estimateProtein(profile) {
...
async function fetchProfile(uid) {
    try {
        ...
        console.warn("coach_plan_profile_error", { uid, message: err?.message });
        return null;
    }
}
...
//# sourceMappingURL=coachPlan.js.map