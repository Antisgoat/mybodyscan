...
function parseCheckoutSessionPayload(data) {
    const body = (data || {});
...
async function createCheckoutSessionForUid(uid, payload) {
    const { plan } = payload;
...
export async function createCheckoutSessionHandler(data, context) {
    ...
    try {
        ...
        if (err instanceof CheckoutError) {
            ...
            throw new HttpsError(code, err.kind);
        }
        throw err;
    }
}
export const createCheckoutSession = onCall({ region: "us-central1", secrets: ["STRIPE_SECRET", "STRIPE_SECRET_KEY"] }, async (request) => createCheckoutSessionHandler(request.data, request));
