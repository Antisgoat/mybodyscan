...
async function handler(req, res) {
    ...
    const gateData = gateSnap.exists ? gateSnap.data() : {};
    ...
    for (const docSnap of recentQuery.docs) {
        ...
        const docHashes = Array.isArray(data.imageHashes) ? data.imageHashes : [];
        if (!docHashes.length)
            continue;
...
export const beginPaidScan = onRequest({ invoker: "public", concurrency: 10 }, withCors(async (req, res) => {
    try {
