...
function toInputRecord(payload) {
    const record = {};
...
function parseOpenAiResponse(content) {
    ...
    try {
        ...
        console.error("scan_parse_json_error", { message: err?.message });
...
async function toImageMeta(uid, scanId, pose) {
...
function buildUserContext(inputs) {
    const parts = [];
    if (inputs.weightLb)
        parts.push(`Weight: ${inputs.weightLb.toFixed(1)} lb`);
    if (inputs.heightIn)
        parts.push(`Height: ${inputs.heightIn.toFixed(1)} in`);
    if (inputs.age)
        parts.push(`Age: ${inputs.age} years`);
    if (inputs.sex)
        parts.push(`Sex: ${inputs.sex}`);
    if (!parts.length)
        return "No additional numeric context provided.";
    return parts.join("; ");
}
async function callOpenAi(images, inputs) {
    ...
    const systemPrompt = "You are a body-composition assistant. Estimate body fat percentage from photographs. Provide only the requested JSON.";
...
async function deleteUploads(paths) {
    await Promise.allSettled(paths.map(async (path) => {
        try {
            ...
            console.warn("scan_cleanup_error", { path, message: err?.message });
        }
    }));
}
async function isFounder(uid) {
    try {
        ...
        console.warn("scan_submit_founder_lookup_error", { uid, message: err?.message });
        return false;
    }
}
export const submitScan = onRequest({ invoker: "public", concurrency: 10, region: "us-central1" }, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.warn("scan_submit_missing_images", { uid, scanId: payload.scanId, message: err?.message });
        ...
        try {
            ...
            result = await callOpenAi(imageDetails.map(({ meta, url }) => ({ pose: meta.pose, url })), payload);
        }
        catch (err) {
