...
function normalizeDate(dateISO, offsetMins) {
...
function sanitizeServing(raw) {
    ...
    const unitOrNull = (value) => typeof value === "string" && value.trim().length ? value.trim().slice(0, 40) : null;
    return {
...
function sanitizeNutrients(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    const numberOrNull = (value) => {
        const num = Number(value);
...
async function upsertMeal(uid, day, meal) {
    const docRef = db.doc(`users/${uid}/nutritionLogs/${day}`);
    let totals = defaultTotals();
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(docRef);
        const data = snap.exists ? snap.data() : { meals: [], totals };
...
async function removeMeal(uid, day, mealId) {
    const docRef = db.doc(`users/${uid}/nutritionLogs/${day}`);
    let totals = defaultTotals();
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(docRef);
...
async function readDailyLog(uid, day) {
...
async function handleAddMeal(req, res) {
    ...
    const meal = {
        ...
        entrySource: typeof body.meal.entrySource === "string"
...
async function handleDeleteMeal(req, res) {
    const uid = await requireAuth(req);
    const body = req.body;
...
async function handleGetLog(req, res) {
    const uid = await requireAuth(req);
    const dateISO = req.body?.dateISO || req.query?.dateISO;
    if (!dateISO) {
...
async function handleGetHistory(req, res) {
    const uid = await requireAuth(req);
    const rangeRaw = req.body?.range ?? req.query?.range ?? 30;
    const range = Math.min(30, Math.max(1, Number(rangeRaw) || 30));
    const anchorIso = req.body?.anchorDateISO || req.query?.anchorDateISO ||
